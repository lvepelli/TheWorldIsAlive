...
- **Live URL (mirror, active now):** https://cdn.jsdelivr.net/gh/lvepelli/TheWorldIsAlive@gh-pages/index.html — jsDelivr serving the `gh-pages` branch; CI purges it on every deploy and also records a commit-pinned mirror URL in `docs/qa/LIVE_URL.txt`. Service workers may not register on the CDN origin (the app still works; just no offline/PWA there).
