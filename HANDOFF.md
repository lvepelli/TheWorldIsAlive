...
5. **Cultural championships** (film festivals, prizes) as recurring calendar events. Passed-over executives already become rivals of the new CEO (`characters.ts`). Religious revival/decline and schisms now live in `spawn.ts` (`religion.tide`, `religion.schism`); a state-religion / theocracy consequence would extend them.
