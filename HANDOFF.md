...
2. **Dialogue depth**: `LLMDialogueProvider` and conversation memories exist (`ai/llm.ts`, `ai/dialogue.ts`); next: let interviews change opinions (a persuaded leader shifts approval or policy), and let journalists quote what characters told the player.
...
4. **Weather with consequences**: storm cells and the Climate overlay are cosmetic (`renderer.ts` `drawWeather`); next, tie them to the disaster spawner so hurricanes and floods strike where a front is parked, and add seasons (monsoon bands, winter in the north).
