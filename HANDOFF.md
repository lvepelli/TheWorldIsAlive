...
2. **Dialogue depth**: `LLMDialogueProvider`, conversation memories, next-day "Interview" posts and persuasion ("you should …" can change an open character's objective) exist in `ai/dialogue.ts`; leaders now act on peace/election/reform/resignation objectives (`characters.ts`); next: objectives for executives and scientists (a persuaded CEO pivots the company), and journalists quoting interviews in articles.
