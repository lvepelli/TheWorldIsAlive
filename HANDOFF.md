...
- **Live URL (mirror, active now):** https://raw.githack.com/lvepelli/TheWorldIsAlive/gh-pages/index.html — raw.githack.com serving the `gh-pages` branch (jsDelivr was tried first but serves HTML as text/plain); CI records a commit-pinned `rawcdn.githack.com` URL in `docs/qa/LIVE_URL.txt`. Service workers may not register on the CDN origin (the app still works; just no offline/PWA there).
