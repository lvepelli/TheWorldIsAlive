...
- **Regions** exist (2–4 per country, own unrest/identity/autonomy, autonomy → concession/crackdown → secession) but have no map rendering of their own (no region borders or overlay) and no regional economy; `Region` carries cities only.
...
6. **Regions** exist (`generator/regions.ts`, `simulation/regions.ts`, rules `region.response`/`region.movement`/`region.secession`; `createCountry(..., regionId)` splits along Voronoi cells of the region's cities). Next: draw region borders (thin dotted lines inside countries) and a Regions overlay coloured by unrest; give regions a leader/governor character; let God Mode target a region ("X region declares independence").
