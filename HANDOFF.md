...
2. **Dialogue depth**: `LLMDialogueProvider`, conversation memories, next-day "Interview" posts and persuasion ("you should …" can change an open character's objective) exist in `ai/dialogue.ts`; next: make objectives drive actions (a leader persuaded to "make peace" should actually seek a ceasefire) and let journalists quote interviews in articles.
