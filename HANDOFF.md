...
3. **More relationship-driven rules**: rival pounce / ally rally / mentor endorsement / purge (`consequences.ts`), breakups and funder withdrawals (`spawn.ts`) exist; mentors turning on protégés exists (`spawn.ts` `mentor.turns`); next: family feuds over inheritance and characters remembering who helped them (use `memories`).
