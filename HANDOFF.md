...
- **Live URL (mirror, active now):** https://cdn.statically.io/gh/lvepelli/TheWorldIsAlive/gh-pages/index.html — statically.io serving the `gh-pages` branch (jsDelivr serves HTML as text/plain; githack works but shows a "One more step" interstitial). CI records commit-pinned mirror URLs in `docs/qa/LIVE_URL.txt`. Statically caches branch refs for a while, so the pinned URL in LIVE_URL.txt is the freshest. Service workers may not register on the CDN origin (the app still works; just no offline/PWA there).
