...
4. **Weather depth**: storm cells steer hurricanes and floods, dry seasons steer droughts and wildfires, droughts move grain prices, and the daily summary carries a forecast (`simulation/weather.ts`, `spawn.ts` disaster rule). Next: crop yields feeding country happiness and a food-price crisis chain when grain spikes.
