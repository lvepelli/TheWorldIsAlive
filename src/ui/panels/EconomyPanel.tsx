      ...
      <div className="table-wrap"><table className="table">
        ...
        <tbody>{sorted.map(({ c, mk, top }) => (
          <tr key={c.id} className="clickable" onClick={() => open(c)} tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter') open(c); }}>
            <td>{c.name}{c.atWarWith.length ? ' ⚔' : ''}</td><td className="num">{fmtMoneyB(c.gdp)}</td><td className="num" style={{ color: trendColor(c.gdpGrowth - 1.5, 0.3) }}>{fmtPct(c.gdpGrowth)}</td><td className="num" style={{ color: c.debt > 120 ? 'var(--bad)' : c.debt > 90 ? 'var(--warn)' : undefined }}>{c.debt.toFixed(0)}%</td><td className="num">{c.unemployment.toFixed(1)}%</td><td className="num" style={{ color: c.inflation > 8 ? 'var(--bad)' : undefined }}>{c.inflation.toFixed(1)}%</td><td className="num" style={{ color: trendColor(mk) }}>{trendArrow(mk)} {fmtPct(mk)}</td><td>{top ? <button className="link" onClick={(e) => { e.stopPropagation(); select({ kind: 'company', id: top.id }); }}>{top.name}</button> : '—'}</td>
          </tr>))}</tbody>
      </table></div>
    </Drawer>
  );
