...
function CityView({ c }: { c: City }): React.ReactElement {
  ...
  return (
    <>
      <div className="row" style={{ gap: 12 }}><div className="grow"><div className="title">{c.name} {c.capital && <span className="tag" style={{ color: 'var(--accent)' }}>capital</span>}</div><div className="dim" style={{ fontSize: 12 }}>{country?.name}{regionOf(world, c.id) ? ` · ${regionOf(world, c.id)!.name} (unrest ${regionOf(world, c.id)!.unrest.toFixed(0)}${regionOf(world, c.id)!.autonomy >= 50 ? ', autonomous' : ''})` : ''} · {c.coastal ? 'coastal' : 'inland'} · {c.specialties.join(', ')}</div></div><button className="btn icon sm" onClick={() => { focusOn(c.x, c.y, 4); setScreen('world'); }}>◎</button></div>
