...
import { regionsOf, regionOf } from '@/engine/generator/regions';
...
function CityView({ c }: { c: City }): React.ReactElement {
  ...
  return (
    <>
      <div className="row" style={{ gap: 12 }}><div className="grow"><div className="title">{c.name} {c.capital && <span className="tag" style={{ color: 'var(--accent)' }}>capital</span>}</div><div className="dim" style={{ fontSize: 12 }}>{country?.name}{regionOf(world, c.id) ? ` · ${regionOf(world, c.id)!.name}` : ''} · {c.coastal ? 'coastal' : 'inland'} · {c.specialties.join(', ')}</div></div><button className="btn icon sm" onClick={() => { focusOn(c.x, c.y, 4); setScreen('world'); }}>◎</button></div>
