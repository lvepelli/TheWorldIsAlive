    ...
    <>
      ...
      {tab === 'overview' && (
        <>
          ...
          {(() => { const regions = regionsOf(world, c).sort((x, y) => y.unrest - x.unrest); if (regions.length < 2) return null; return <Section title="Regions" right={<span className="row" style={{ gap: 8 }}><span className="dim mono">{regions.length}</span><button className="btn ghost sm" title="Colour the map by regional unrest" onClick={() => { useGame.getState().setOverlay('regions'); focusOn(c.centroid.x, c.centroid.y, 2.2); setScreen('world'); }}>Show on map</button></span>}><div className="list">{regions.map((r) => { const cap = r.cityIds.includes(c.capitalId); return <div key={r.id} className="entity-row" onClick={() => useGame.getState().select({ kind: 'region', id: r.id })} role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter') useGame.getState().select({ kind: 'region', id: r.id }); }}><div className="grow"><div>{r.name}{cap && <span className="tag" style={{ color: 'var(--accent)' }}>capital</span>}{r.autonomy >= 50 && <span className="tag">autonomous</span>}</div><div className="dim" style={{ fontSize: 12 }}>{r.cityIds.length} {r.cityIds.length === 1 ? 'city' : 'cities'} · {fmtPop(regionStats(world, r).population)} · prosperity {regionStats(world, r).prosperity.toFixed(0)} · identity {(r.identity * 100).toFixed(0)}% · autonomy {r.autonomy.toFixed(0)}{r.governorId && world.people[r.governorId] ? <> · <span className="link" onClick={(e) => { e.stopPropagation(); useGame.getState().select({ kind: 'person', id: r.governorId! }); }}>Gov. {world.people[r.governorId].name}</span></> : ''}{r.history.length ? ` · ${r.history[r.history.length - 1].text.replace(/\.$/, '').toLowerCase()}` : ''}</div></div><div style={{ minWidth: 72, textAlign: 'right' }}><div className="mono" style={{ fontSize: 12, color: barColor(100 - r.unrest) }}>unrest {r.unrest.toFixed(0)}</div><div className="bar" style={{ marginTop: 4 }}><i style={{ width: `${r.unrest}%`, background: barColor(100 - r.unrest) }} /></div></div></div>; })}</div></Section>; })()}
...
function RegionView({ r }: { r: Region }): React.ReactElement {
  const world = useGame((s) => s.world)!; const focusOn = useGame((s) => s.focusOn); const setScreen = useGame((s) => s.setScreen); const version = useGame((s) => s.version);
  ...
  return (
    <>
      ...
      {gov && <Section title="Governor"><EntityRow refx={{ kind: 'person', id: gov.id }} name={gov.name} sub={`${gov.title ?? 'Politician'} · wants to ${gov.objective}`} /></Section>}
      <Section title="Cities"><div className="list">{cities.map((c) => <EntityRow key={c.id} refx={{ kind: 'city', id: c.id }} name={c.name} sub={`${fmtPop(c.population)} · prosperity ${c.prosperity.toFixed(0)}${c.capital ? ' · capital' : ''}`} />)}</div></Section>
      {r.history.length > 0 && <Section title="Story"><div className="list">{r.history.slice().reverse().slice(0, 8).map((h, i) => <div key={i} className="row" style={{ gap: 8, fontSize: 12 }}><span className="dim mono">{formatDate(h.day, world.meta.startYear, 'short')}</span><span>{h.text}</span></div>)}</div></Section>}
      {events.length > 0 && <Section title="Events here"><div className="list">{events.map((e) => <EventCard key={e.id} ev={e} compact />)}</div></Section>}
    </>
  );
}
...
function CityView({ c }: { c: City }): React.ReactElement {
  ...
  return (
    <>
      <div className="row" style={{ gap: 12 }}><div className="grow"><div className="title">{c.name} {c.capital && <span className="tag" style={{ color: 'var(--accent)' }}>capital</span>}</div><div className="dim" style={{ fontSize: 12 }}>{country?.name}{regionOf(world, c.id) ? <> · <span className="link" onClick={() => useGame.getState().select({ kind: 'region', id: regionOf(world, c.id)!.id })}>{regionOf(world, c.id)!.name}</span></> : ''} · {c.coastal ? 'coastal' : 'inland'} · {c.specialties.join(', ')}</div></div><button className="btn icon sm" onClick={() => { focusOn(c.x, c.y, 4); setScreen('world'); }}>◎</button></div>
    ...
    <>
      ...
      {tab === 'profile' && (
        <>
          ...
          <Section title="Character"><div className="chips">{p.traits.map((t) => <span key={t} className="chip">{t}</span>)}<span className="chip">ambition {(p.personality.ambition * 100).toFixed(0)}</span><span className="chip">integrity {(p.personality.integrity * 100).toFixed(0)}</span><span className="chip">charisma {(p.personality.charisma * 100).toFixed(0)}</span></div><div className="muted" style={{ fontSize: 13, marginTop: 6 }}>Objective: <b>{p.objective}</b>{(() => { const h = p.history.slice().reverse().find((x) => x.text.startsWith('Persuaded by an interviewer') || x.text.startsWith('Steered ')); return h && world.day - h.day < 400 ? <span className="dim" style={{ fontSize: 12 }}> · {h.text.startsWith('Persuaded') ? 'changed course after an interview' : 'acted on it'} {world.day - h.day === 0 ? 'today' : `${world.day - h.day}d ago`}</span> : null; })()}</div></Section>
          <Dialogue p={p} />
          ...
          {p.profession === 'journalist' && (() => { const arts = world.news.filter((n) => n.authorId === p.id).slice(-5).reverse(); return arts.length ? <Section title="Bylines"><div className="list">{arts.map((a) => <div key={a.id} className="card clickable" onClick={() => useGame.getState().select({ kind: 'event', id: a.eventId })}><div style={{ fontFamily: 'var(--font-serif)', fontWeight: 700 }}>{a.headline}</div><div className="dim" style={{ fontSize: 11 }}>{world.outlets[a.outletId]?.name} · {daysAgo(a.day, world.day)}</div></div>)}</div></Section> : null; })()}
        ...
        <>
          <Section title="Why did this happen?" right={<span className="dim">{total} downstream</span>}>
            <div className="chain">
              ...
              {chain.map((c) => <React.Fragment key={c.id}><div className="node" style={{ ['--c' as string]: catVar(c.category) }} onClick={() => select({ kind: 'event', id: c.id })}><span className={`sev sev-${c.severity}`} role="img" aria-label={sevLabel(c.severity)} title={sevLabel(c.severity)} /><span className="grow ellipsis">{c.title}</span><span className="dim mono" style={{ fontSize: 10 }}>{daysAgo(c.day, world.day)}</span></div><div className="arrow">↓ led to</div></React.Fragment>)}
              <div className="node current" style={{ ['--c' as string]: catVar(ev.category) }}><span className={`sev sev-${ev.severity}`} role="img" aria-label={sevLabel(ev.severity)} title={sevLabel(ev.severity)} /><span className="grow">{ev.title}</span></div>
              {consequences.map((c) => <React.Fragment key={c.id}><div className="arrow">↓ caused</div><div className="node" style={{ ['--c' as string]: catVar(c.category) }} onClick={() => select({ kind: 'event', id: c.id })}><span className={`sev sev-${c.severity}`} role="img" aria-label={sevLabel(c.severity)} title={sevLabel(c.severity)} /><span className="grow ellipsis">{c.title}</span><span className="dim mono" style={{ fontSize: 10 }}>{daysAgo(c.day, world.day)}</span></div></React.Fragment>)}
...
function Dialogue({ p }: { p: Person }): React.ReactElement {
  ...
  const ask = async (question: string) => { if (!question.trim()) return; const a = await dialogueProvider.answer(world, p, question); const r = rememberConversation(world, p, question, a); useGame.getState().bump(); setLog((l) => [...l.slice(-5), { q: question, a: r.persuaded ? `${a}\n✦ ${p.firstName} took it to heart: new objective “${r.persuaded}”.` : a }]); setQ(''); };
  return (
