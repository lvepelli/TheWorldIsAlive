...
export function sagaTitle(world: { countries: Record<string, { name: string }> }, root: { type: string; title: string; location: { countryId?: string }; actors: { kind: string; id: string }[]; data?: Record<string, unknown> }, chain: { type: string }[]): string {
  ...
  if (root.type === 'region.autonomy') { const rn = root.data?.region as string | undefined; if (types.has('region.referendum')) return `The ${rn ?? 'regional'} referendum`; return types.has('country.created') || Array.from(types).some((t) => t.startsWith('country.')) ? `The birth of ${rn ?? 'a new nation'}` : types.has('region.crackdown') ? `The ${rn ?? 'regional'} crackdown` : `The ${rn ?? 'regional'} question`; }
