...
function examplesFor(world: NonNullable<ReturnType<typeof useGame.getState>['world']>): string[] {
  ...
  return [
    ...
    ...(region ? [`Hold a referendum in ${region.name}.`] : []),
            ...
            <div className="grid-2">
              {preset.params.map((prm) => (
                <div key={prm.key} className="field">
                  ...
                  {prm.type === 'text' && prm.key !== 'region' && <input className="input" value={params[prm.key] ?? ''} onChange={(e) => setParams({ ...params, [prm.key]: e.target.value })} placeholder={prm.optional ? 'optional' : ''} />}
                  {prm.type === 'text' && prm.key === 'region' && (() => { const owner = world.countries[(preset.id === 'annex' ? params.b : params.a) ?? '']; const names = owner ? (owner.regionIds ?? []).map((id) => world.regions?.[id]).filter(Boolean).map((r) => r!.name) : Object.values(world.regions ?? {}).map((r) => r.name).slice(0, 200); return <><input className="input" list={`regions-${preset.id}`} value={params[prm.key] ?? ''} onChange={(e) => setParams({ ...params, [prm.key]: e.target.value })} placeholder={owner ? `a region of ${owner.name}` : 'type a region name'} /><datalist id={`regions-${preset.id}`}>{names.map((n) => <option key={n} value={n} />)}</datalist></>; })()}
                </div>
              ))}
