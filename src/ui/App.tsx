      ...
      <div className={`app ${selection ? 'with-inspector' : ''}`}>
        ...
        {more && (
          <Modal title="More" onClose={() => setMore(false)}>
            ...
            <div className="row wrap"><button className="btn" onClick={() => { setMore(false); setSaves(true); }}>💾 Save / load / export</button><button className="btn" onClick={() => { const w = useGame.getState().world; if (!w) return; const url = `${location.origin}${location.pathname}?seed=${encodeURIComponent(w.meta.seed)}`; const share = (navigator as Navigator & { share?: (d: { title: string; text: string; url: string }) => Promise<void> }).share; if (share) void share.call(navigator, { title: 'The World Is Alive', text: `Explore my world "${w.meta.name}"`, url }).catch(() => undefined); else { navigator.clipboard?.writeText(url).then(() => alert('Link copied')).catch(() => prompt('Copy this link', url)); } }}>🔗 Share this world</button><InstallButton /></div>
