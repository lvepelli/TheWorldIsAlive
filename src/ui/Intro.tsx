        ...
        {phase === 'generating' ? (
          ...
          <>
            ...
            <div className="col" style={{ width: '100%', gap: 10 }}>
              ...
              <div className="chips scroll" aria-label="Featured worlds" style={{ justifyContent: 'safe center' }}>{FEATURED_SEEDS.map((f) => { const pr = PREMISES.find((x) => x.id === f.premise); return <button key={f.seed} className={`chip clickable ${seed === f.seed ? 'active' : ''}`} title={pr?.blurb} onClick={() => setSeed(f.seed)}>{pr?.title ?? f.premise}</button>; })}</div>
