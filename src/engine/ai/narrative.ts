...
export function isNegative(ev: WorldEvent): boolean {
  ...
  return /collapse|crisis|crash|scandal|coup|bankrupt|protest|strike|death|assassin|crackdown|flop|failure|downfall|tension\.rise|cyber|migration|clash|alliance\.broken|rival\.attack|purge|must go|breakup|withdraws|schism|religion\.decline/.test(ev.type + ' ' + ev.title.toLowerCase());
}
