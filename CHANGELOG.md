...
## 1.0.0 — the grand-strategy overhaul (Spanish-first)

- **Language**: Spanish is the default; English is selectable in Settings and on the title screen (persisted). All interface text goes through `src/i18n` (≈780 keys per language, parity-tested). Engine events are rendered in Spanish at display time (`engine/i18n/render.ts`, ~115 event families plus disasters; English fallback), dates and relative days are localized, God presets carry Spanish labels and descriptions, and God commands and character questions can be typed in Spanish (`godmode/es.ts` keyword bridge). News articles, social posts, summaries and character answers are still generated in English (documented in HANDOFF).
- **Text hygiene**: `engine/text.ts` `tidy()` runs on every generated title, description, headline and post (repeated words and phrases, double articles, space before punctuation, `..`, `??`, empty parentheses, repeated sentences); `findTextIssues()` lints; `tests/text.test.ts` scans a simulated world; the E2E runs scan the rendered screen.
- **Layout**: top bar (date, speeds, jump menu, world indicators, search, alerts, menu) · left section rail · the map always underneath · sections open as a column beside the map (wide for economy, diplomacy, events, history, news, God, calendar) · tabbed entity panels with breadcrumbs (Mundo › País › Región › Ciudad) · bottom navigation and sheets on phones.
- **Map**: twelve named map modes (político, regiones, economía, población, diplomacia, conflictos, religión, tecnología, estabilidad, recursos, empresas, migración) with hints; differentiated country colours, quieter borders, label collision avoidance, region labels only in Regions mode or at high zoom.
- **Sections**: Countries, Regions, People, Companies (searchable, sortable lists) · Economy (sortable world table → click opens the country and focuses the map; global index and commodities; approximations flagged) · Politics · Diplomacy (alliances, wars, rivalries, trade, per-country relation groups) · Technology · Society · Religions · Calendar (today / month / year / upcoming from the real schedules) · Events (what just happened, developing stories, timeline filtered by date, country, region, category, severity and text; priority labels Crítico / Importante / Normal / Menor) · Alerts (grouped by war, economy, politics, science, society, disaster; read / dismiss / open).
- **Entity panels**: country tabs (resumen, economía with income/spending/exports/imports/confidence/industries/resources, política, regiones, empresas, diplomacia, sociedad, personas, historia, eventos), region and city views, person tabs (resumen, relaciones, carrera, eventos, opinión with dialogue), company tabs (resumen, finanzas, red, eventos), event view with *¿Por qué ha ocurrido?* / *¿Qué ha causado?*, impact (economic vs political) and reactions.
- **God Mode**: interventions regrouped by player-facing category (política, guerra, economía, sociedad, tecnología, medio ambiente, empresas, personas, regiones, global); every preset now has target, magnitude and timing controls, a preview with expected severity, immediate effects and possible consequences; free text accepts Spanish; nothing was removed.
- **Performance**: the 20-year balance run stays within ~10 % of the pre-overhaul engine (text hygiene is a linear token scan).
- **Tests**: `tests/i18n.test.ts` (dictionary parity, ≥ 90 % of simulated events localized without template leaks, dates, Spanish God commands) and `tests/text.test.ts`; the E2E scripts use stable `data-testid` / `data-nav` hooks and check sections, map modes, economy table, events, God categories, alerts, ES ↔ EN switching, text duplication and persistence on desktop and three phone viewports.
- Removed the unused Live, Markets, Orgs and People screens (their content lives in the Events, Economy, Society/Religions and People panels).
